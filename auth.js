...
function favKey(email){ return `${LS_FAV_PREFIX}${email}`; }
function loadFavs(email){
  if(!email) return new Set();
  try { return new Set(JSON.parse(localStorage.getItem(favKey(email))) || []); }
...
function hydrateTopbar(){
  ...
  if(sess?.email){
    ...
    if(btnLogin){ btnLogin.hidden = TrueIf(btnLogin.getAttribute('data-hide-when-logged') || '1'); }
    if(btnLogout){ btnLogout.hidden = false; btnLogout.addEventListener('click', (e)=>{ e.preventDefault(); logoutAndRedirect(); }); }
  ...
  function TrueIf(v){ return String(v) === '1'; }
}
